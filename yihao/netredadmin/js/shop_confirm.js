$(function () {
	...
	function getApplyList(data) {
		var url = $.baseUrl + '/Back/Index/auditor_shop_list';
		$.ajax({
			url: url,
			type: 'POST',
			data: data,
	...
	function setData(data) {
		var el = $('#tbody')
		if (data.auditor_shop_list) {
			el.empty();
			$.each(data.auditor_shop_list, function (i, val) {
				var tr = $('<tr>').data('info', val);
				var td0 = $('<td>').text(val.name);
				var td1 = $('<td>').text(val.create_at);
				var txt = '';
				var td3 = $('<td>');
	...
	function getConfirmInfo(data) {
		var url = $.baseUrl + '/Back/Index/auditor_shop_info'
		$.ajax({
			url: url,
			type: 'POST',
			data: data,
	...
	function setConfirmInfo(data) {
		...
		var a = data.shop_info;
		console.log(a);
		$('#shop_id').val(a.shop_id);
		$('#refuse').data('shop_id',a.shop_id).data('user_id',a.user_id);
		$('#pass').data('shop_id',a.shop_id).data('user_id',a.user_id);
		$('#shop_name1').val(a.shop_name);
		$('#address').val(a.address);
		$('#username').val(a.admin_name);
		$('#phone').val(a.admin_phone);
		$('#shop_description').text(a.shop_describe);
		var arr = data.shop_img;
		$('#shop_image').empty();
		for(var i in arr){
			console.log(arr[i]);
			var img = $('<img>',{src:arr[i].img_path}).data(arr[i]);
			$('#shop_image').append(img);
		}
	};
	...
	$("#pass").on('click', function () {
		...
		console.log(data);
		wxc.xcConfirm("是否审核通过", wxc.xcConfirm.typeEnum.confirm, {
			onOk: function () {
				apply_refuse(data);
			}
		})
	...
	function apply_refuse(data) {
		$.ajax({
			url:$.baseUrl + '/Back/Index/auditor_shop',
			type: 'POST',
			data: data,
			dataType: $.dataType,
			success: function (res) {
	...
	/*查看图片*/
	$('#shop_image').on('click', 'img', function () {
		$('.imgPopver').show();
		setImg(this.src);
	});
	function setImg(src) {
